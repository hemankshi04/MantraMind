...
============================================================
  ...
  REQUIRES:
    model/best_model.pkl  (run train_enhanced_model.py first)
============================================================
"""
...
def predict():
    ...
    try:
        ...
        mantra = MANTRA_MAP.get(d['mantra'], d['mantra'])
        if mantra not in le_mantra.classes_:
            mantra = le_mantra.classes_[0]

        gender = GENDER_MAP.get(d['gender'], d['gender'])
        ...
        return jsonify({
            ...
            'metrics': {
                ...
                'stress_reduction':  round(sr,  1),
                'anxiety_reduction': round(ar,  1),
                'focus_gain':        round(fg,  1),
                'calm_gain':         round(cg,  1),
                'hrv_change':        hrv,
